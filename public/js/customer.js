window.onload = function () {
  ...
  const register_customer = document.getElementById("customer_register");
  register_customer.addEventListener("click", registerCustomer);
  ...
  function getFirstFormData() {
    ...
    let mobile_number_one = document.getElementsByName("phone")[0].value;
  ...
  //     document.getElementsByName("business_address")[0].value;
  //   let data = {
  ...
  async function registerCustomer(e) {
    try {
      ...
      if (!formData) {
        ...
        register_customer.innerHTML = "Loading...";
        const response =  await fetch(`${host}/user/customer/signup`, {
          method: "POST",
        ...
        register_customer.innerHTML = "Register";
